...
def create_augmented_dataloader(args, dataset):
    ...
    # Create augmented training dataset by adding 5,000 transformed examples
    # to the original training data
    
    # Get the original training dataset
    original_train = dataset["train"]
    
    # Sample 5,000 random examples from training set for transformation
    # Use shuffle to randomize, then select first 5000
    sampled_train = original_train.shuffle(seed=42).select(range(5000))
    
    # Apply the custom transformation to these 5,000 examples
    transformed_train = sampled_train.map(custom_transform, load_from_cache_file=False)
    
    # Combine original training data with the 5,000 transformed examples
    # This creates a dataset of size 25,000 (original) + 5,000 (transformed) = 30,000
    from datasets import concatenate_datasets
    augmented_train = concatenate_datasets([original_train, transformed_train])
    ...
    # Prepare for model (remove text column, rename label)
    ...
    print(f"Augmented training dataset size: {len(augmented_tokenized)}")
    print(f"Original: 25,000 + Transformed: 5,000 = Total: {len(augmented_tokenized)}")
